...
class LocalInferencePipeline(torch.nn.Module):
    def __init__(self, ckpt_path, device="cuda"):
        ...
        # Load the MediaPipe modules from your local 'preparation' directory
        print("🤖 Initializing MediaPipe face & landmark trackers...")
        ...
        print(f"🧠 Loading model weights into GPU memory from: {ckpt_path}")
    ...
    def forward(self, data_filename):
        ...
        print("✂️ Tracking face landmarks and isolating the lip matrix...")
        ...
        print("🔮 [GPU] Decoding visual lip matrix movements into text...")
        with torch.no_grad():
...
def overlay_captions(input_video_path, output_video_path, caption_text):
    ...
    print("🎬 Burning prediction strings into video tracks...")
    cap = cv2.VideoCapture(input_video_path)
    if not cap.isOpened():
        print("❌ Error: Frame buffering stream failed.")
        return
    ...
    cap.release()
    out.release()
    print(f"✅ Operations complete! Caption file created at: {output_video_path}")
...
if __name__ == "__main__":
    ...
    print("🚀 Initializing GPU Environment Diagnostics...")
    
    # Force check for CUDA capability
    if not torch.cuda.is_available():
        print("\n❌ CRITICAL ERROR: CUDA GPU environment acceleration is not accessible.")
    ...
    print(f"✅ CUDA GPU Verified! Using: {torch.cuda.get_device_name(0)}")
    runtime_device = "cuda"

    if not os.path.exists(INPUT_VIDEO_FILE):
        print(f"❌ Error: Input video file '{INPUT_VIDEO_FILE}' was not found in your directory.")
        ...
        try:
            ...
            print(f"❌ Execution failure during runtime: {error_instance}")