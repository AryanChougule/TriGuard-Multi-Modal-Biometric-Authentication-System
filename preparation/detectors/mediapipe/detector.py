...
class LandmarksDetector:
    ...
    def __call__(self, video_frames, cancel_event=None):
        landmarks = self.detect(video_frames, self.full_range_detector, cancel_event=cancel_event)
        if all(element is None for element in landmarks):
            landmarks = self.detect(video_frames, self.short_range_detector, cancel_event=cancel_event)
            assert any(l is not None for l in landmarks), "Cannot detect any frames in the video"
        return landmarks

    def detect(self, video_frames, detector, cancel_event=None):
        landmarks = []
